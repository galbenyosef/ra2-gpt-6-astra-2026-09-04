...
test('deployment guard excludes converted media while allowing application workers and WASM',()=>{
  for(const path of ['assets/manifest.json','maps/catalog.json','app/gacnst.png','app/menu.webm','app/menu.mp4','app/hm2.wav','app/source.MIX','public/README.md','.cache/installer.exe'])assert.equal(forbiddenBuildPath(path),true,path);

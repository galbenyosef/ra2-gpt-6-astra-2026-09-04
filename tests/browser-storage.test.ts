...
import { sidebarAssets } from '../src/hud/skin';
...
test('browser readiness requires a committed cache and every original file, including audio and maps',async()=>{
  ...
  files.push('/assets/audio/hm2.wav','/maps/mp22s8.map', ...sidebarAssets.map(({key})=>`/assets/ui/${key}.png`));
  ...
  try{
    ...
    for(const missing of ['/assets/audio/hm2.wav','/maps/mp22s8.map','/assets/ui/sidec02-gclock2.png','/assets/ui/sidec01-tab00.png']){

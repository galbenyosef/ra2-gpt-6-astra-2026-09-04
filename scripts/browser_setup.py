...
with sync_playwright() as p:
    ...
    if page.locator('#setup-download').count():
        ...
        last=''
        for i in range(1200):
            if page.get_by_test_id('mode-skirmish').count():
                # Successful conversion reloads into mode selection before the lobby.
    ...
    before=len([r for r in requests if 'archive.org' in r]);page.reload(wait_until='networkidle');page.get_by_test_id('mode-skirmish').click(timeout=90000);page.wait_for_selector('#start',timeout=90000)
    assert len([r for r in requests if 'archive.org' in r])==before,'Re-downloaded on repeat visit'
    context.set_offline(True);page.reload(wait_until='networkidle');page.get_by_test_id('mode-skirmish').click(timeout=90000);page.wait_for_selector('#start',timeout=90000)

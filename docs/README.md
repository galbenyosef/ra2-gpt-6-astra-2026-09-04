<!-- User guides and reproducible acceptance records; screenshots remain local. -->
# Guides and verification

```text
game-guide.md                  gameplay, assets and editor
save-games.md                  save controls, format and regression checks
verification.md                earlier application acceptance
bootcamp*-verification.md      model switching, terrain and training
sidebar-verification.md        native faction chrome and production
menus-verification.md          Rust Alarm entry/ESC, commands, hover and live options
menu-video-verification.md     original Bink playback, browser conversion and Fira fonts
```

Run commands from the repository root. Original-media evidence stays in ignored
`.cache/` paths referenced by each report; it is never committed or published.

...
import{actors}from'../environment/canvas3d/catalog.js';
...
try{const page=await browser.newPage({viewport:{width:1440,height:1000}}),errors=[];page.on('pageerror',e=>errors.push(e.message));await page.goto((process.env.RA2_BROWSER_URL||'http://127.0.0.1:4192')+'/canvas3d/');await page.waitForFunction(()=>window.canvas3d?.ready);await page.locator('#demo').click();await page.evaluate(()=>window.canvas3d.pause(true));
...
await fs.writeFile(out+'/behavior.json',JSON.stringify({...report,errors},null,2));assert.equal(report.missing.length,0);assert.deepEqual(report.models,actors.map(a=>a.id));assert.equal(errors.length,0);

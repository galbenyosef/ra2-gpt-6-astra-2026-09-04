...
export async function createCanvasPreview({base='/',httpServer,repo=process.env.RA2_REPO||path.resolve(root,'../..')}={}){
 ...
 return createServer({configFile:false,root,base,publicDir:false,resolve:{alias:{'@game':repo+'/src','three/addons':threeRoot+'/examples/jsm','three':threeRoot+'/build/three.module.js'}},define:{__BUILD_INFO__:JSON.stringify({hash:'local-hd-preview',committedAt:null})},server:{allowedHosts:process.env.RA2_PREVIEW_HOST?[process.env.RA2_PREVIEW_HOST]:[],...(httpServer?{middlewareMode:true,hmr:{server:httpServer}}:{host:'127.0.0.1',port:4177,strictPort:true}),fs:{allow:[root,repo+'/src',repo+'/node_modules']}},plugins:[{name:'local-preview-art',configureServer(server){server.middlewares.use((req,res,next)=>{
  ...
  if(url==='/original-manifest.json'||url==='/sprites/manifest.json'){res.setHeader('Content-Type','application/json');res.end(JSON.stringify(url==='/sprites/manifest.json'?JSON.parse(readFileSync(path.join(art,'sprites/manifest.json'),'utf8')):{sprites:selected,terrain,scenery,overlays,mapMetadata,terrainMap,error}));return;}

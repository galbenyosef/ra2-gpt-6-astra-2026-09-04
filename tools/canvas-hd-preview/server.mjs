...
export async function createCanvasPreview({base='/',httpServer,repo=process.env.RA2_REPO||path.resolve(root,'../..')}={}){
 ...
 const modelRoot=process.env.RA2_MODEL_DIR||path.join(repo,'.cache/hifi-branch/hifi/assets');
 const models={mtnk:path.join(art,'models/apocalypse-tank/30k.glb'),tany:path.join(modelRoot,'tanya/tanya.glb'),gacnst:path.join(modelRoot,'allied-construction-yard/allied-construction-yard.glb'),nanrct:path.join(modelRoot,'soviet-nuclear-power-plant/soviet-nuclear-power-plant.glb')};

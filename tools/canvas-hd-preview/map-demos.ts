...
export function createMapDemos(game:GameEngine,renderer:BattlefieldRenderer,sprites:Record<string,Sprite>,groundHeight:(x:number,y:number)=>number){
 ...
 let attacking=false,waterAttacking=false,phase=0,waterPhase=0,height=0;
 ...
 return {entities,update(time:number){
  phase=time%8;waterPhase=time%12;const shouldAttack=phase>=3&&phase<6,shouldWaterAttack=waterPhase>=10;
  ...
  const swimT=Math.min(waterPhase,8)/8;swim.x=16.8+3.6*Math.sin(Math.PI*swimT);swim.y=11.5;swim.angle=waterPhase<4||waterPhase>=8?0:Math.PI;

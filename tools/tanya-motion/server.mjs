...
const port=Number(process.env.PORT||4179),hdRoot=path.resolve(repo,process.env.RA2_HD_ASSETS||'assets/hd');
...
const staticFiles=new Map(['index.html','viewer.mjs','motion.css','catalog.mjs','bake-atlas.mjs','locomotion.mjs','team-material.mjs','crawl-keys.mjs','action-keys.mjs'].map(n=>['/'+n,path.join(root,n)]));staticFiles.set('/',root+'/index.html');staticFiles.set('/tanya-actions.glb',hdRoot+'/models/tanya/tanya-actions.glb');staticFiles.set('/original-tanya.png',art+'/sprites/tany.png');
let canvas;const server=http.createServer(async(req,res)=>{try{
 const p=new URL(req.url,'http://localhost').pathname;
 if(p==='/bake-result'&&req.method==='POST'){
  if(req.headers.origin!==`http://127.0.0.1:${port}`||req.headers['cf-connecting-ip']){res.writeHead(403).end();return;}
  let size=0,chunks=[];for await(const c of req){size+=c.length;if(size>64*1024*1024)throw Error('Bake body too large');chunks.push(c);}const b=JSON.parse(Buffer.concat(chunks));const dir=hdRoot+'/sprites';
  ...
  if(b.metadata?.sourceFrames!==619||b.metadata.frames<619||b.metadata.frames>4096||b.metadata.frameRects?.length!==b.metadata.frames)throw Error('Incorrect frame mapping');const m=JSON.parse(await fs.readFile(dir+'/manifest.json'));m.sprites.tany=b.metadata;await fs.writeFile(dir+'/manifest.json',JSON.stringify(m,null,2)+'\n');await fs.mkdir(repo+'/.cache/motion',{recursive:true});await fs.writeFile(repo+'/.cache/motion/bake-bounds.json',JSON.stringify(b.bounds));res.writeHead(200).end('Saved');return;
 }
...
const{createCanvasPreview}=await import('../canvas-hd-preview/server.mjs');canvas=await createCanvasPreview({base:'/canvas/',repo,httpServer:server});server.listen(port,'127.0.0.1',()=>console.log('SHP motion preview ready http://127.0.0.1:'+port+'/'));

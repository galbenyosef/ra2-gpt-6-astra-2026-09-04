...
- 默认发布高清参考图、生成记录与约 3 万面的轻量 GLB；原始 GLB 仅作本地优化输入和按需留存的母版。保存输入图顺序、路径、哈希以及实际提示词、模型版本、参数、task ID；已有缺失信息标为未知，不虚构。具体留存规则见 [主技能](../SKILL.md#2-轻量与可恢复性)。
...
生成网格不等于绑定完成。对适合的双足模型，复用已有 rig 或按当前接口执行授权的绑定任务，再核对 joints、clips、权重与返回材质；具体见 [骨骼适配](skeletal-motion.md)。车辆与建筑不走人形自动绑定。谭雅已成功的本地实例见 [动作记录](tanya-motion-example.md)，其缓存与历史参数不是新请求的默认有效 schema。
